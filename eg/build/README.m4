...
\include(DOCM4_DIR/short-copyright-notice.m4)
 ...
 by which a program is transformed from source to executable form,
 aka the build process.

 Examples follow an envisaged sequence of incremental enhancements
 originally meant to guide the student throughout increasingly
 sophisticated topics, each built upon the preceding concepts.
 This is however not the only possible learning path; one may
 follow a different order as it fits better.
